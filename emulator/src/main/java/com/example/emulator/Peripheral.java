...
public class Peripheral extends Activity implements ServiceFragment.ServiceFragmentDelegate {
  ...
  private static final UUID CLIENT_CHARACTERISTIC_CONFIGURATION_UUID = UUID
    ...
    public final static String ACTION_GATT_CONNECTION =
            "com.example.bluetooth.le.ACTION_GATT_CONNECTION";
      ...
      if (status == BluetoothGatt.GATT_SUCCESS) {
        if (newState == BluetoothGatt.STATE_CONNECTED) {
          ...
          Log.v(TAG, "Connected to device: " + device.getAddress());
            broadcastUpdate(ACTION_GATT_CONNECTION,true);
        } else if (newState == BluetoothGatt.STATE_DISCONNECTED) {
          mBluetoothDevices.remove(device);
          updateConnectedDevicesStatus();
          Log.v(TAG, "Disconnected from device");
            broadcastUpdate(ACTION_GATT_CONNECTION,false);
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      mCurrentServiceFragment = new EmulatorFragment();
      getFragmentManager()
          .beginTransaction()
  ...
  private void disconnectFromDevices() {
    ...
    Fragment frg = null;
    frg = getFragmentManager().findFragmentByTag(CURRENT_FRAGMENT_TAG);
    ...
    private void broadcastUpdate(final String action, boolean isConnected) {
        ...
        intent.putExtra(CONNECTION_STATUS,isConnected);
